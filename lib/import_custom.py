...
def find_molecules(img_original, x_pixels, y_pixels, background_1=0.15, background_2=0.25, min_area=300, manual=False,
    ...
    # EDITTED: Extract the individual molecules
    molecules = []
    ...
    for region in measure.regionprops(img_labelled):
        ...
        # Pad each molecule with a 10-pixel border of the original image or 0 if at the edges of the original image
        if np.amin([minr, minc]) < 10 or maxr > (x_pixels - 10) or maxc > (y_pixels - 10):
            curr_molecule.append(np.pad(img_original[minr:maxr, minc:maxc], pad_width=10, mode='constant'))
        else:
            curr_molecule.append(img_original[minr - 10:maxr + 10, minc - 10:maxc + 10])
    
        # Pad the filtered box as well
        curr_molecule.append(np.pad(mol_filtered_box, pad_width=10, mode='constant'))
