...
def create_custom_colormap(midpoint=0.35):
    """ 
    Define a self-made colormap starting at #375E97 going to 1.0 and ending at #D61800.
    If 'my_colormap' is already registered, generate a new name and register the new colormap.
    """
    
    # Define the color dictionary for the colormap
    color_dict = {
        'red': ((0.0, 0.0, 0.261),
                (midpoint, 1.0, 1.0),
                (1.0, 0.839, 0)),
    ...
    base_name = 'my_colormap'
    my_colormap = LinearSegmentedColormap(base_name, color_dict)
    
    # Check if the colormap is already registered
    colormap_name = base_name
    counter = 1
    while colormap_name in plt.colormaps():
        colormap_name = f"{base_name}_{counter}"
        counter += 1
    
    # Register the colormap with a unique name
    matplotlib.colormaps.register(my_colormap, name =colormap_name)
...
def convert_to_rgb(mol_filtered, my_colormap_name):
    """ Converts a greyscale image to RGB and scales it properly to a colormap"""
    ...
    # Retrieve the colormap object using the name
    my_colormap = plt.cm.get_cmap(my_colormap_name)

    # Convert the greyscale image to a color image
    mol_filtered_rgba = my_colormap(mol_filtered)
    mol_filtered_rgb = np.delete(mol_filtered_rgba, 3, 2)  # Remove alpha channel
